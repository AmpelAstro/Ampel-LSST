#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : Ampel-ZTF/ampel/lsst/t2/T2CatalogMatch.py
...
class T2CatalogMatch(CatalogMatchUnit, AbsPointT2Unit):
    ...
    # Dict of coordinate mappings (for other instruments, e.g. dec for ZTF and decl for LSST)
    coorddict: Dict[str, str] = {
            'ra': 'ra',
            'dec': 'dec'
        }
    ...
    def process(self, datapoint: DataPoint) -> Union[UBson, UnitResult]:
        ...
        try:
            transient_ra = datapoint["body"][self.coorddict["ra"]]
            transient_dec = datapoint["body"][self.coorddict["dec"]]
        except KeyError:
